(function (exports, ByteArray, global) {
  ...
  Package.decode = function(buffer){
    ...
    while(offset < bytes.length) {
      var buff, origin, len, length;
      if(offset !== 0) {
        origin = buff;
        len = buff.length - offset;
        buff = new ByteArray(len);
        copyArray(buff, 0, origin, offset, len);
      } else {
        buff = bytes;
      }
      var type = buff[0];
      var index = 1;
      length = ((buff[index++]) << 16 | (buff[index++]) << 8 | buff[index++]) >>> 0;
      offset += length + PKG_HEAD_BYTES;
      var body = length ? new ByteArray(length) : null;
      copyArray(body, 0, buff, PKG_HEAD_BYTES, length);
